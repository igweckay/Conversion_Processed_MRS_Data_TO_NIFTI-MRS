...
opts.exportParams.flag      = 1;                % Options:    - 0 (no, default)
                                                %             - 1 (yes)
% exportParams.path is set per-subject inside the data loop below
...
opts.fit.basisSetFile = 'C:\Users\Kay\Desktop\JuchemLab\Software\osprey-develop\osprey-develop\fit\basissets\3T\siemens\unedited\slaser\26\ospreybasis_marss_to_cows_26';
...
for kk = 1:length(subs)
    ...
    for ll = 1:length(sess)
        ...
        % (MANDATORY)
        dir_metabolite    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_OCC.nii.gz']);
        files(counter)      = {[dir_metabolite(end).folder filesep dir_metabolite(end).name]};

        % Set per-subject export path for fitting parameters
        opts.exportParams.path{counter} = fullfile(sess(ll).folder, sess(ll).name, 'mrs', 'osprey_params');
